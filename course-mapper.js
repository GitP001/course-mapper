...
// ========== small safety: show errors instead of blank page ==========
window.addEventListener("error", (e) => {
  const root = document.getElementById("root");
  if (root && !root.dataset.hasError) {
    root.dataset.hasError = "1";
    root.innerHTML =
      '<div class="max-w-xl mx-auto mt-10 p-5 rounded-xl bg-rose-50 border border-rose-200 text-rose-700">' +
        ...
        if (typeof catVal === "string") used.add(catVal);
          ...
          for (const v of Object.values(catVal)) {
            if (typeof v === "string") used.add(v);
              ...
              v.forEach((x) => typeof x === "string" && used.add(x));
          }
        }
      }
    }
...
/** Special renderer for Section 2B remaining */
...
/** NEW: Special renderer for Section 2C remaining */
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        <div className="text-xs uppercase tracking-wide text-gray-500">Completed so far</div>
        <span className="text-sm rounded-full px-2 py-0.5 bg-gray-100">{completed_so_far}</span>
        <div className="text-xs uppercase tracking-wide text-gray-500 ml-4">Needed</div>
        <span className="text-sm rounded-full px-2 py-0.5 bg-gray-100">{needed}</span>
      </div>

      <div className="mt-2">
        <div className="text-sm font-semibold mb-1">Categories matched</div>
        {Object.keys(categories_matched).length ? (
          <div className="flex flex-wrap">
            {Object.entries(categories_matched).map(([cat, list]) =>
              (list || []).map((course, i) => (
                <Chip key={cat + i} text={`${course} (${cat})`} />
              ))
            )}
          </div>
        ) : (
          <div className="text-gray-400 text-sm">None yet — add more electives.</div>
        )}
      </div>
    </div>
  );
...
function computeSuggestions(remaining) {
  ...
  for (const sec of order) {
    ...
    } else if (sec === "2C" && typeof rem === "object") {
      const pool = Object.entries(rem.categories_matched || {}).flatMap(([cat, list]) =>
        (list || []).map((c) => ({ section: sec, category: cat, course: c }))
      );
      picks.push({ section: sec, need: rem.needed ?? 0, options: pool });
