...
// === Behavior: hide taken courses from "Remaining" everywhere ===============
const HIDE_TAKEN_IN_REMAINING = true;
...
// === Embedded requirement presets (your JSONs) ===============================
...
function evaluateUserCourses(userCourses, requirementData) {
  ...
  const removeTaken = (list) =>
    HIDE_TAKEN_IN_REMAINING ? list.filter((c) => !userCourses.includes(c)) : list.slice();
  ...
  // ---------- 1B ----------
  {
    ...
    if (rule.type === "multi-category-limit") {
      ...
      for (const cat of rule.valid_categories || []) {
        ...
        if (matched.length) c[cat] = matched.slice(0, rule.max_per_category || matched.length);
        r[cat] = removeTaken(list);
      }
  ...
  // ---------- 2B (flatten subjects) ----------
  {
    ...
    if (rule.type === "subgroup-selection") {
      const c = {}, r = {};
      const groups = Object.keys(sec).filter((g) => !isMeta(g));
      for (const groupName of groups) {
        ...
        // Remaining options = subject lists minus taken
        for (const [subject, list] of Object.entries(group)) {
          r[`${groupName} — ${subject}`] = removeTaken(list);
        }
      }
  ...
  // ---------- 2C (no double count in Completed; Remaining hides taken) ------
  {
    ...
    if (rule.type === "elective-geoscience") {
      ...
      for (const key of ["1A", "1B", "2A", "2B"]) {
        ...
        if (!secComp) continue;
        for (const val of Object.values(secComp)) if (Array.isArray(val)) val.forEach((x) => used.add(x));
      }

      const c = {}, r = {};
      for (const [subject, list] of Object.entries(sec)) {
        if (isMeta(subject)) continue;
        const validTaken = [...new Set(list.filter((x) => userCourses.includes(x) && !used.has(x)))];
        if (validTaken.length) c[subject] = validTaken;
        r[subject] = removeTaken(list);
      }
...
const Chip = ({ text, onRemove }) => (
  <span className="inline-flex items-center gap-2 bg-gray-100 rounded-full px-3 py-1 text-sm shadow-sm mr-2 mb-2">
    ...
    {onRemove && <button onClick={onRemove} className="hover:opacity-70" aria-label={`Remove ${text}`}>&times;</button>}
  </span>
);
    ...
    <div className="space-y-2">
      {Object.entries(data).map(([cat, arr]) => (
        <div key={cat}>
          ...
          <div className="mt-1">{(arr || []).map((v, i) => <Chip key={cat + i} text={String(v)} />)}</div>
        </div>
      ))}
    </div>
  );
    ...
    <div className="rounded-2xl shadow p-5 bg-white border border-gray-100">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-xs text-slate-600">
          <span>Required: {metrics.required}</span><Badge>Done: {metrics.done}</Badge><Badge>Need: {metrics.need}</Badge>
        </div>
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        <div><h4 className="font-medium text-green-600 mb-2">Completed</h4><ListBlock data={completed} /></div>
        <div><h4 className="font-medium text-rose-600 mb-2">Remaining</h4><ListBlock data={remaining} /></div>
      </div>
    </div>
  );
}
...
function CourseMapperApp() {
  ...
  const [rawCourses, setRawCourses] = useLocal("cm:courses","CHEM 120, MATH 127, PHYS 111, CS 115, MATH 114, PHYS 112, STAT 202, EARTH 231, EARTH 235, EARTH 238, EARTH 221, EARTH 260, EARTH 342");
  ...
  useEffect(() => {
    try {
      ...
      if (oldRaw) {
        ...
        localStorage.removeItem("cm:reqs:v2"); localStorage.removeItem("cm:reqs");
      } else { setRequirementsMap((prev) => ({ ...DEFAULT_REQS, ...prev })); }
    } catch {}
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-900">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <header className="grid md:grid-cols-3 gap-6 mb-8">
          <div className="p-5 bg-white rounded-2xl shadow border border-gray-100">
            ...
            <div className="mt-3">
              {courses.length ? (
                <div className="flex flex-wrap">
                  {courses.map((c) => (
                    <span key={c} className="inline-flex items-center gap-2 bg-gray-100 rounded-full px-3 py-1 text-sm shadow-sm mr-2 mb-2">
                      <span className="font-medium tracking-wide">{c}</span>
                      <button onClick={() => setRawCourses(courses.filter((x) => x !== c).join(", "))} className="hover:opacity-70" aria-label={`Remove ${c}`}>&times;</button>
                    </span>
                  ))}
                </div>
