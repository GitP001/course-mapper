/* Course Mapper App (UMD, no JSX/Babel required) */
(function () {
  ...
  // === Embedded requirement presets (your finalized JSONs) ===================
  ...
  const Chip = ({ text, onRemove }) =>
    h(
      ...
      h("span", { className: "font-medium tracking-wide" }, text),
      onRemove
        ? h(
            "button",
  ...
  function Remaining2B({ remaining }) {
    if (!remaining || !Object.keys(remaining).length) {
      return h("div", { className: "text-gray-400 text-sm" }, "None");
    }
    return h(
      "div",
      { className: "space-y-4" },
            ...
            ...Object.entries(data.remaining_subjects || {}).map(([subject, list]) =>
              h(
                ...
                h(
                  ...
                  h("span", { className: "font-medium" }, `${subject}:`),
                  h("span", { className: "ml-2" }, joinList(list))
                )
              )
            ),
            !Object.keys(data.remaining_subjects || {}).length
              ? h("div", { className: "text-gray-400 text-xs" }, "No remaining subjects.")
              : null
          )
        )
      )
  ...
  function Completed2B({ completed }) {
    ...
    if (!has) return h("div", { className: "text-gray-400 text-sm" }, "None");
    return h(
      "div",
      { className: "space-y-2" },
      ...Object.entries(completed).map(([groupName, arr]) =>
        h(
          "div",
          { key: groupName },
          h("div", { className: "text-sm font-semibold mb-1" }, groupName),
          h(
            "div",
            { className: "mt-1" },
            ...(Array.isArray(arr) ? arr : []).map((obj, i) => {
              const pair = Object.entries(obj)[0] || ["", ""];
              return h(Chip, { key: groupName + i, text: `${pair[0]}: ${pair[1]}` });
            })
          )
        )
      )
  ...
  function Remaining2C({ remaining }) {
    ...
    const { completed_so_far, needed, categories_matched = {} } = remaining;
    return h(
      "div",
      { className: "space-y-2" },
      h(
        "div",
        null,
        h("span", { className: "text-xs uppercase tracking-wide text-gray-500 mr-2" }, "Completed so far"),
        h(Chip, { text: String(completed_so_far ?? 0) })
      ),
      h(
        "div",
        null,
        h("span", { className: "text-xs uppercase tracking-wide text-gray-500 mr-2" }, "Needed"),
        h(Chip, { text: String(needed ?? 0) })
      ),
      h("div", { className: "text-sm font-semibold mt-2" }, "Categories matched"),
      h(
        "div",
        { className: "mt-1" },
        ...Object.entries(categories_matched).flatMap(([cat, list], i) => [
          h("div", { key: `cm-${i}`, className: "text-xs uppercase tracking-wide text-gray-500 mr-2 mb-1" }, cat),
          ...(Array.isArray(list) ? list : []).map((c, j) => h(Chip, { key: `cm-${i}-${j}`, text: c }))
        ])
      )
    );
  }
    ...
    return h(
      ...
      ...Object.entries(data).map(([cat, val]) =>
        h(
          "div",
          { key: cat },
          h("div", { className: "text-sm font-semibold" }, cat),
          h(
            "div",
            { className: "mt-1" },
            Array.isArray(val)
              ? val.map((v, i) => h(Chip, { key: cat + i, text: String(v) }))
              : typeof val === "object"
              ? Object.entries(val).map(([k, v]) => h(Chip, { key: cat + k, text: `${k}: ${v}` }))
              : h(Chip, { text: String(val) })
          )
        )
      )
    );
  ...
  function SectionCard({ title, sectionKey, completed = {}, remaining = {} }) {
    ...
    return h(
      ...
      h("h3", { className: "text-lg font-semibold mb-3" }, title),
      h(
        "div",
        ...
        h(
          "div",
          null,
          h("h4", { className: "font-medium text-green-600 mb-2" }, "Completed"),
          is2B ? h(Completed2B, { completed }) : h(GenericList, { data: completed })
        ),
        h(
          "div",
          null,
          h("h4", { className: "font-medium text-rose-600 mb-2" }, "Remaining"),
          is2B ? h(Remaining2B, { remaining }) : is2C ? h(Remaining2C, { remaining }) : h(GenericList, { data: remaining })
        )
      )
    );
  }
  ...
  function runSelfTests() {
    ...
    }
    {
      ...
      const need = remaining?.["1B"]?.needed_more_courses;
      const pass = need === 3;
      results.push({ name: "1B min total", pass, details: { need } });
    }
    {
      ...
      const pass = !!(g1 && g1.length && g2 && g2.length && g3 && g3.length) && !remaining?.["2B"];
      results.push({ name: "2B subgroup coverage", pass, details: completed["2B"] });
    }
    {
      const courses = ["EARTH 231", "EARTH 223"];
      const { completed } = evaluateUserCourses(courses, GEOSCIENCE);
      const usedIn2A = !!completed?.["2A"]?.["Mineralogy and Petrology"];
      const count2C = completed?.["2C"] ? Object.values(completed["2C"]).flat().length : 0;
      const pass = usedIn2A && count2C === 1;
      results.push({ name: "2C no double-counting", pass, details: { usedIn2A, count2C } });
    }
    {
      const courses = ["EARTH 221", "EARTH 331", "EARTH 333"];
      const { completed } = evaluateUserCourses(courses, GEOSCIENCE);
      const streams = determineStream(completed);
      const pass = streams.includes("Geology");
      results.push({ name: "Stream detection: Geology", pass, details: { streams } });
    }
    return results;
  }
  ...
  function CourseMapperApp() {
    ...
    const completed = evalRes.completed;
    const remaining = evalRes.remaining;
      ...
      h(
        ...
        h(
          ...
          h(
            "div",
            null,
            h("h1", { className: "text-2xl md:text-3xl font-extrabold tracking-tight" }, "Course Mapping Planner"),
            h(
              "p",
          ...
          h(
            ...
            h(
              ...
              h("span", { className: "text-sm font-medium" }, "Upload requirements JSON"),
              h("input", { type: "file", accept: "application/json", onChange: onUpload, className: "hidden" })
            ),
            h(
              "button",
            ...
            h(
              "button",
              { onClick: exportPlan, className: "px-3 py-2 rounded-2xl bg-slate-900 text-white text-sm shadow hover:opacity-90" },
              "Export Plan"
            )
          )
        ),
        ...
        h(
          ...
          h(
            ...
            h("label", { className: "block text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2" }, "Program"),
            h(
              "select",
              {
                value: program,
              ...
              ...Object.keys(requirementsMap).map((name) => h("option", { key: name, value: name }, name))
            ),
            h(
              "p",
          ...
          h(
            ...
            h(
              ...
              courses.length
                ...
                : h("span", { className: "text-slate-400 text-sm" }, "No courses parsed yet.")
            )
          )
        ),
        ...
        !reqData
          ...
          : h(
              ...
              h(
                ...
                h(SectionCard, { title: "Section 1A", sectionKey: "1A", completed: completed["1A"], remaining: remaining["1A"] }),
                h(SectionCard, { title: "Section 1B", sectionKey: "1B", completed: completed["1B"], remaining: remaining["1B"] }),
                h(SectionCard, { title: "Section 2A", sectionKey: "2A", completed: completed["2A"], remaining: remaining["2A"] }),
                h(SectionCard, { title: "Section 2B", sectionKey: "2B", completed: completed["2B"], remaining: remaining["2B"] }),
                h(SectionCard, { title: "Section 2C", sectionKey: "2C", completed: completed["2C"], remaining: remaining["2C"] })
              ),
              ...
              program.toLowerCase() === "geoscience"
                ? h(
                    ...
                    h("h3", { className: "text-lg font-semibold mb-2" }, "Detected PGO Stream(s)"),
                    streams.length
                      ? h(
                          "div",
                          { className: "flex flex-wrap" },
                          ...streams.map((s) => h(Chip, { key: s, text: s }))
                        )
                      : h("div", { className: "text-slate-500 text-sm" }, "Could not determine stream. More relevant 2B/2C courses may be needed.")
                  )
                : null,
              ...
              h(
                "section",
                { className: "mb-8 p-5 bg-white rounded-2xl shadow border border-gray-100" },
                h("h3", { className: "text-lg font-semibold mb-2" }, "Suggested Next Picks"),
                suggestions.length
                  ? h(
                      "div",
                      { className: "space-y-4" },
                      ...suggestions.map((s, i) =>
                        h(
                          ...
                          h(
                            ...
                            h("span", null, s.section),
                            typeof s.need === "number"
                              ? h("span", { className: "text-xs text-slate-500" }, `Need ~ ${s.need}`)
                              : null
                          ),
                          h(
                            "div",
                            { className: "flex flex-wrap" },
                            ...s.options.slice(0, 24).map((o, j) => h(Chip, { key: j, text: o.course + (o.category ? ` (${o.category})` : "") })),
                            s.options.length > 24
                              ? h("span", { className: "text-xs text-slate-500 ml-1" }, `+${s.options.length - 24} more...`)
                              : null
                          )
                        )
                      )
                    )
                  : h("div", { className: "text-slate-500 text-sm" }, "No suggestions — all sections satisfied or missing requirement metadata.")
              ),
              ...
              h(
                ...
                h(
                  ...
                  h("h3", { className: "text-lg font-semibold" }, "Self-tests"),
                  h("button", { onClick: handleRunTests, className: "px-3 py-1.5 rounded-lg border shadow-sm" }, "Run Tests")
                ),
                tests.length
                  ? h(
                      "ul",
                      ...
                      ...tests.map((t, i) =>
                        h(
                          "li",
                          {
                            key: i,
                            className:
                              "p-2 rounded border " + (t.pass ? "border-green-300 bg-green-50" : "border-rose-300 bg-rose-50"),
                          },
                          h("div", { className: "font-medium" }, t.name, " ", t.pass ? "PASS" : "FAIL"),
                          h("pre", { className: "whitespace-pre-wrap text-xs text-slate-600" }, JSON.stringify(t.details, null, 2))
                        )
                      )
                    )
                  : null
  ...
  // Mount app to #root (safe for React 17 or 18)
  (function mount() {
    const mountNode = document.getElementById("root");
    const el = React.createElement(CourseMapperApp);
    if (ReactDOM.createRoot) {
      ReactDOM.createRoot(mountNode).render(el); // React 18+
    } else if (ReactDOM.render) {
      ReactDOM.render(el, mountNode); // React 17 fallback
    } else {
      console.error("Neither ReactDOM.createRoot nor ReactDOM.render found.");
    }
  })();
})();
