...
<html lang="en">
  <head>
    ...
    <title>Course Mapping Planner</title>
  ...
  <body class="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-900">
    ...
    <script type="text/babel">
      ...
      function determineStream(completed) {
        ...
        const geology = new Set(); const env = new Set();
      ...
      const Chip = ({ text, onRemove }) => (
        <span className="inline-flex items-center gap-2 bg-gray-100 rounded-full px-3 py-1 text-sm shadow-sm mr-2 mb-2">
          ...
          {onRemove && (
            <button onClick={onRemove} className="hover:opacity-70" aria-label={\`Remove \${text}\`}>&times;</button>
          )}
        </span>
      );
                            ...
                            ? Object.entries(val).map(([k, v]) => (
                                <div key={cat + k} className="mb-1">
                                  ...
                                  {Array.isArray(v)
                                    ...
                                    : typeof v === "object"
                                    ? Object.entries(v).map(([kk, vv]) => (<Chip key={cat + k + kk} text={\`\${kk}: \${String(vv)}\`} />))
                                    : <Chip text={String(v)} />}
                                </div>
                              ))
                            : <Chip text={String(val)} />}
            ...
            {!reqData ? (
              ...
              <>
                ...
                <section className="mb-8 p-5 bg-white rounded-2xl shadow border border-gray-100">
                  ...
                  {(() => {
                    const suggestions = computeSuggestions(remaining);
